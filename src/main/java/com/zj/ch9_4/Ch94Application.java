...
public class Ch94Application {
	...
	//自动获得https://spring.io/blog.atom的资源
	...
	//FeedEntryMessageSource实际为feed:inbound-channel-adapter，此处即构造feed的入站
	@Bean
	...
	public IntegrationFlow myFlow() throws IOException{
		return IntegrationFlows.from(feedEntryMessageSource()) //流程从from方法开始
				...
				.<SyndEntry,String>route(payload -> payload.getCategories().get(0).getName(), 
						mapping -> mapping.channelMapping("releases","releasesChannel") //通过不同分类的值转向不同的消息通道
	...
	public IntegrationFlow engineeringFlow(){
		return IntegrationFlows.from(MessageChannels.queue("engineeringchannel", 10))
				.<SyndEntry,String>transform(e -> "《"+e.getTitle()+"》"+
	...
	public IntegrationFlow newsFlow(){
		return IntegrationFlows.from(MessageChannels.queue("newsChannel", 10))
				.<SyndEntry,String>transform(payload -> "《"+payload.getTitle()+"》"+
						payload.getLink()+getProperty("line.separator"))
				.enrichHeaders( //增加消息头的信息
						Mail.headers()
						.subject("来自Spring的新闻")
						.to("wisely-man@126.com"))
				.handle(Mail.outboundAdapter("smtp.126.com") //构造邮件发送的相关信息
